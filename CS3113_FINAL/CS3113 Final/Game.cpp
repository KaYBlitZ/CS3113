...
Game::Game() : displayWindow(nullptr), keyStates(nullptr), done(false), state(GameState::MENU), jumpSound(nullptr), respawnSound(nullptr), music(nullptr) {
	...
	setPerspective(60, 1.33f, 3.0f, 30.0f);
...
void Game::render() {
	...
	case GameState::MENU:
		...
		drawText("Hello World", 0.0f, 0.0f, -4.0f, 1.0f);
		break;
...
void Game::setPerspective(float fovY, float aspect, float zNear, float zFar) {
	Matrix m;
	m.identity();
	m.m[0][0] = 1.0f / tanf(fovY * Constants::DEG_TO_RAD / 2.0) / aspect;
	m.m[1][1] = 1.0f / tanf(fovY * Constants::DEG_TO_RAD / 2.0);
...
void Game::drawText(string text, float x, float y, float z, float scale) {
	drawText(fontSheet, 16, 16, text, x, y, z, scale, 1.0f, 1.0f, 1.0f, 1.0f);
}
